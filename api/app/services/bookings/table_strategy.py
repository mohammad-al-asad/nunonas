...
from app.models.booking import RescheduleRequest, TableBookingCreate
from app.services.bookings.base import BookingStrategy


class TableBookingStrategy(BookingStrategy):
    def normalize_for_create(self, payload: TableBookingCreate, listing: dict) -> tuple[dict, datetime]:
        _ = listing
    ...
    def normalize_for_reschedule(self, existing_details: dict, payload: RescheduleRequest) -> tuple[dict, datetime]:
        if payload.time is None:
            raise ValueError("time is required for table reschedule")
